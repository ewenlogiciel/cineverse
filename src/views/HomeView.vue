    ...
    <div v-else>
      ...
      <div v-if="featuredFilm" class="relative h-[80vh] w-full mb-8 group rounded-2xl overflow-hidden">
        <div class="absolute inset-0">
          <img v-if="heroBackdrop" :src="heroBackdrop" class="w-full h-full object-cover object-center" alt="Featured Film"/>
        ...
        <div class="absolute bottom-[20%] left-4 md:left-12 max-w-2xl z-10 space-y-4 px-4">
          <h1 class="text-4xl md:text-6xl font-bold text-white mb-3 truncate drop-shadow-lg leading-tight">{{ featuredFilm.name }}</h1>
          <div class="flex items-center space-x-4 text-sm md:text-base text-gray-300 font-medium">
            <span v-if="featuredFilm.releaseData" class="text-green-400">{{ new Date(featuredFilm.releaseData).getFullYear() }}</span>
            <span v-if="featuredFilm.duration">{{ featuredFilm.duration }} min</span>
          </div>
          <p class="text-gray-300 text-sm md:text-lg line-clamp-3 drop-shadow-md max-w-xl">{{ featuredFilm.description }}</p>
          <div class="flex flex-wrap gap-4 pt-4">
            <router-link :to="`/films/${featuredFilm.id}`" class="flex items-center bg-white/5 border border-gray-700 rounded text-sm text-gray-400 py-3 px-6 font-medium transition-all hover:bg-red-500/10 hover:border-red-500/30 hover:text-red-400">
      ...
      <div class="-mt-24 relative z-20 space-y-12">
        ...
        <div v-if="filmsStore.films.length > 0" data-test="movies-list">
          <h2 class="text-xl md:text-2xl font-bold text-white md:pl-4">
            Films à l'affiche
          </h2>
...
const loadAllFilms = async () => {
  // Vider immédiatement les films pour éviter d'afficher les anciens résultats filtrés
  filmsStore.films = []

  // Toujours recharger tous les films sans filtre ni recherche sur la page d'accueil
  await filmsStore.fetchFilms({ search: undefined, director: undefined, sort: undefined })
  setFeaturedFilm()
}
...
// Recharger tous les films à chaque fois qu'on revient sur la page d'accueil
...
const setFeaturedFilm = async () => {
  if (filmsStore.films.length > 0) {
    // Filtrer les films qui ont une image de l'API
    const filmsWithImage = filmsStore.films.filter(film => film.image || film.poster)

    if (filmsWithImage.length > 0) {
      // Sélectionner un film aléatoire parmi ceux qui ont une image
      const randomIndex = Math.floor(Math.random() * filmsWithImage.length)
      featuredFilm.value = filmsWithImage[randomIndex]
      ...
      try {
        if (featuredFilm.value.name) {
          ...
          if (results && results.length > 0 && results[0].backdrop_path) {
            ...
            // Utiliser l'image de l'API en fallback si TMDB ne trouve rien
            heroBackdrop.value = featuredFilm.value.image || featuredFilm.value.poster
          }
        }
      } catch (e) {
        console.error("Erreur récupération backdrop TMDB", e)
        // Utiliser l'image de l'API en fallback en cas d'erreur
        heroBackdrop.value = featuredFilm.value.image || featuredFilm.value.poster
      }
    } else {
      // Si aucun film n'a d'image, on n'affiche pas de hero
      featuredFilm.value = null
    }
  } else {
    // Si la liste est vide, on retire le film Hero
    featuredFilm.value = null
  }
}
</script>
