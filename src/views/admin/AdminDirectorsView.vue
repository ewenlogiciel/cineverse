<template>
  <div class="max-w-[90rem] mx-auto px-4 py-8">
    ...
    <div v-if="directorsStore.loading" class="flex justify-center items-center py-20">
    ...
    <div v-else-if="directorsStore.directors.length > 0" class="bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-2xl shadow-2xl overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-800">
          ...
          <tr v-for="(director, index) in directorsStore.directors" :key="director.id" class="hover:bg-white/5 transition-colors">
            <td class="px-4 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-500 font-medium">{{ index + 1 }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm font-medium text-white">{{ director.lastname }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-400">{{ director.firstname }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm text-gray-400">
                {{ director.dob ? formatDate(director.dob) : 'N/A' }}
              </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm">
              <div class="flex space-x-4">
                <router-link
                    :to="`/admin/directors/${director.id}/edit`"
    ...
    <div v-else class="text-center py-20 bg-gradient-to-br from-gray-900 to-black border border-gray-800 rounded-2xl shadow-2xl">
      ...
      <p class="text-gray-400 text-2xl mb-4">Aucun réalisateur trouvé</p>
      <router-link
          to="/admin/directors/new"
...
const confirmDelete = async (director) => {
  if (!confirm(`Voulez-vous vraiment supprimer "${director.firstname} ${director.lastname}"?`)) return

  try {
    await directorsStore.deleteDirector(director.id)
  } catch (err) {
    alert('Erreur lors de la suppression du réalisateur')
  }
}
